...
import { updateListCardOrder, updateBoardListOrder } from '../../helper/index';
...
export const Board = props =>{
  ...
  const InnerList = ({listsData, index}) => {
    return <List
      ...
      cards={
        isEmpty(cards) ? [] :
        listsData.card_order.map(card => cards[card])
      }
      index={index}
    >
  ...
  const onDragEnd = (result, provided) => {
    ...
    if(type === 'list') {
      ...
      updateBoardListOrder(boardId, listArray.map(list => list._id));
    ...
    if(source.droppableId === destination.droppableId) {
      const droppableList = newLists.find(list => list._id === source.droppableId);
      ...
      const listSource = newLists.find(list => list._id === source.droppableId);
      const listDestination = newLists.find(list => list._id === destination.droppableId);
